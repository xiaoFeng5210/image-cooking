...
2. **输入文件**必填。`-o` 可选，不写则输出到当前目录的 `output.jpeg`。
3. 执行命令。成功会打印压缩后大小（按 `B` / `KB` / `MB` / `GB` 自动换算）；若输出比原图更大，会提示改用 `--size`。

### 最常用

不指定输出路径（默认写成 `output.jpeg`）：

```bash
image-cooking photo.jpg --size 1280
```
...
| `-o`, `--output`  | 否   | `output.jpeg` | 输出文件路径。不传则写到当前目录的 `output.jpeg`。会**新建或覆盖**这个文件。 |
...
当前目录有 `background.png`，不传 `-o` 时压完写到 `output.jpeg`，质量 90，最长边 600：

```bash
cd ~/Pictures
image-cooking background.png -q 90 --size 600
```
...
```bash
cargo run -- background.png -q 90 --size 600
```
...
- **会覆盖已有文件。** `-o` 指向的路径（或不传时的 `output.jpeg`）如果已存在，会被直接覆盖。
